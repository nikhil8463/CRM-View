  ...
  const handleSubmit = (e) => {
    ...
    createCampaign(campaignData, {
      onSuccess: async (response) => {
        // If user is assigned, assign them after campaign creation
        if (formData.assigned_user_id) {
          try {
            await campaignService.assignUsers(response.campaign.id, [parseInt(formData.assigned_user_id)])
            toast.success('Campaign created and user assigned successfully!')
          } catch (error) {
            toast.warning('Campaign created, but user assignment failed')
          }
        } else {
          toast.success('Campaign created successfully!')
        }
        navigate(`/dashboard/campaigns/${response.campaign.id}`)
      },
      onError: (error) => {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="body-text text-slate-600 mt-1">Set up a new marketing campaign</p>
          </div>
        </div>
      </div>
